...
\begin{table}[H]
  ...
  \label{table:bonn_dataset}
    {\small
    \begin{tabular}{|>{\raggedright\arraybackslash}p{2.0cm}|>{\raggedright\arraybackslash}p{1.6cm}|>{\raggedright\arraybackslash}p{5.0cm}|>{\raggedright\arraybackslash}p{2.8cm}|>{\raggedright\arraybackslash}p{1.7cm}|}
    \hline
    \textbf{Subset} & \textbf{Label} & \textbf{Description} & \textbf{Recording Type} & \textbf{Count} \\
    \hline
...
\caption{Representative EEG signal waveforms from the five subsets of the Bonn dataset. Seizure (S) signals show distinctly higher amplitudes and rhythmic patterns compared to normal (Z, O) signals.}
\label{fig:signal_comparison}
\end{figure}

The frequency-domain characteristics further highlight the differences between subsets. Figure~\ref{fig:fft_comparison} shows the Fast Fourier Transform (FFT) magnitude spectra for each class. Seizure recordings concentrate energy in the low-frequency delta and theta bands (0.5--8~Hz), while normal recordings show broader spectral distributions with prominent alpha-band (8--13~Hz) activity, particularly in the eyes-closed condition (Set O).

\begin{figure}[H]
\centering
\includegraphics[width=\textwidth]{figures/fft_comparison.png}
\caption{FFT frequency spectra for the five EEG subsets, revealing distinct spectral signatures}
\label{fig:fft_comparison}
\end{figure}

Figure~\ref{fig:amplitude_distribution} shows the amplitude distributions for each subset. The clear separation between Normal ($<$200~$\mu$V) and Seizure ($>$1000~$\mu$V) amplitudes explains why binary classification is straightforward, while the significant overlap between N and F distributions explains why five-class classification is much harder.

\begin{figure}[H]
\centering
\includegraphics[width=\textwidth]{figures/amplitude_distribution.png}
\caption{Amplitude distribution histograms for the five EEG subsets}
\label{fig:amplitude_distribution}
\end{figure}
...
After correction, the binary accuracy decreased from 99.95\% to 99.67\%, and the five-class accuracy from 86.60\% to 84.51\%, which is consistent with the removal of optimistic bias introduced by the earlier leakage-prone pipeline.
...
Figure~\ref{fig:preprocessing_pipeline} summarises the complete preprocessing and evaluation pipeline.

\begin{figure}[H]
\centering
\includegraphics[width=\textwidth]{figures/preprocessing_pipeline.png}
\caption{Preprocessing and evaluation pipeline. The red dashed box highlights the recording-level split that prevents data leakage. Sliding window segmentation and normalisation are applied independently to training and validation sets.}
...
\begin{table}[H]
  ...
  \label{table:model_architecture}
    {\small
    \begin{tabular}{|>{\raggedright\arraybackslash}p{2.8cm}|>{\raggedright\arraybackslash}p{6.5cm}|>{\raggedright\arraybackslash}p{4.0cm}|}
    \hline
...
Figure~\ref{fig:model_architecture} provides a visual overview of the hybrid model architecture, showing the data flow from input through each component to the final classification output.

\begin{figure}[H]
\centering
\includegraphics[width=0.85\textwidth]{figures/model_architecture.png}
\caption{Architecture of the proposed hybrid 1D-CNN + Bi-LSTM + Self-Attention model. Tensor shapes are annotated at each stage. The CNN extracts local features (blue), the Bi-LSTM models temporal dependencies (green), the Self-Attention assigns importance weights (orange), and the classifier produces the final output (grey).}
...
The output of the CNN is passed to two stacked Bi-LSTM layers with 128 and 64 hidden units respectively. Each Bi-LSTM processes the sequence in both forward and backward directions, producing output vectors of dimension $2 \times \text{hidden\_units}$ at each time step. This bidirectional processing captures both past and future temporal context, which can help model the evolution of seizure dynamics over time.
...
For binary classification, the classifier outputs a single logit, which is optimised with Binary Cross-Entropy with Logits Loss; the sigmoid transformation is therefore handled inside the loss function rather than by a separate output activation. For three-class and five-class tasks, the output layer has $N$ neurons (where $N$ is the number of classes) and is trained with Cross-Entropy Loss, which applies softmax implicitly. The total number of trainable parameters is approximately 504{,}000 (1.92~MB).
...
\begin{table}[H]
  ...
  \label{table:baselines}
    {\small
    \begin{tabular}{|>{\raggedright\arraybackslash}p{3.0cm}|>{\raggedright\arraybackslash}p{6.0cm}|>{\raggedright\arraybackslash}p{4.0cm}|}
    \hline
...
All deep learning experiments were conducted on an NVIDIA GeForce RTX 3070 Ti GPU (8~GB VRAM) using PyTorch 2.x with CUDA acceleration. The framework was migrated from TensorFlow/Keras because native Windows GPU support was discontinued in TensorFlow 2.11 and later versions. Under the local setup used in this project, that change enabled GPU training and reduced the measured wall-clock time from roughly $\sim$55~s to $\sim$0.7~s per training epoch.
