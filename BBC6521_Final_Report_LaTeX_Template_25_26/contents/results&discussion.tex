...
Three methodological choices probably contribute to this gap. First, the hybrid model keeps each 1024-point window as a continuous time series rather than flattening it into fixed-length feature vectors, which preserves the temporal context that the Bi-LSTM and attention components rely on. Second, recording-level splitting --- while it eliminates overlap-induced data leakage --- does not prevent the model from learning useful generalisable features; the model still reaches 84.51\% without any obvious leakage signal, which suggests that performance is driven by genuine pattern recognition rather than by memorising specific recordings. Third, the ablation protocol in Section~4.1 shows that no single component (CNN-only, Bi-LSTM-only) reaches the full hybrid accuracy, which is consistent with the interpretation that both local spectral features and longer-range temporal structure are needed for the five-class decision boundary.

\subsection{Discussion of Failure Cases and Per-class Observations}

Beyond the aggregated accuracy numbers, several per-class observations from the confusion matrices in Section~4.2 deserve an explicit discussion, because they reveal where the hybrid model is most and least reliable.

\textbf{Seizure (S) class: near-perfect detection.} Across all three classification tasks and all four models, Set~S is almost never misclassified. This is not surprising given the amplitude gap between ictal and non-ictal EEG (roughly an order of magnitude on the Bonn dataset), but it is a useful sanity check: if the model failed to detect the most visually obvious class, it would signal a pipeline bug rather than a modelling shortcoming.

\textbf{Inter-ictal N vs.\ F: the dominant five-class confusion.} Set~N (inter-ictal, contralateral to the epileptogenic zone) and Set~F (inter-ictal, within the epileptogenic zone) are both recorded from epilepsy patients in their seizure-free state. The neurophysiological difference between these two classes is subtle: it reflects the spatial origin of the recording electrodes relative to the seizure focus, rather than any global difference in the signal. The fact that the hybrid model still achieves usable N/F separation is encouraging, but the residual confusion in the N$\leftrightarrow$F cells of Figure~\ref{fig:cm_five_hybrid} is the single largest contributor to the drop from 97.69\% (three-class) to 84.51\% (five-class). A multi-channel dataset with explicit spatial information would likely narrow this gap.

\textbf{Normal Z vs.\ O: a smaller but consistent confusion.} Set~Z (eyes open) and Set~O (eyes closed) differ mainly in the presence or absence of the posterior alpha rhythm (8--13~Hz). Because the Bonn recordings are single-channel, this alpha difference is the only cue available, and the Bi-LSTM + attention component does pick it up in most folds. The few misclassifications here are plausibly caused by recordings where the alpha rhythm is less pronounced, e.g., under mild drowsiness.

\textbf{Fold-to-fold variance.} The individual fold curves in Figures~\ref{fig:tc_binary_fold}, \ref{fig:tc_three_fold}, and \ref{fig:tc_five_fold} show that not all folds are equally easy. For binary classification, Fold~6 (97.1\%) is a clear outlier; for five-class, Fold~4 (78.9\%) is the lower bound. Inspection of the fold assignments suggests that folds with a higher concentration of F recordings in the validation set tend to be the more difficult ones, which is consistent with the N/F confusion pattern above. This cross-fold variance is a natural consequence of recording-level splitting on a small dataset, and it is a useful reminder not to over-interpret any single-fold number.
...
\begin{itemize}
    ...
    \item \textbf{Absence of per-file subject identifiers}: The Bonn dataset was recorded from 5 subjects per subset, but the public release does not provide a per-file mapping from recordings to subject identities \cite{andrzejak2001}. As a result, all cross-validation in this work is performed at the recording level rather than at the subject level. A strict subject-wise (leave-subjects-out) evaluation --- which would better reflect the case of deploying the model on previously unseen patients --- is therefore not feasible with Bonn alone. This limitation was raised during supervision, and the agreed arrangement is to retain the recording-level protocol for the Bonn-based evaluation and to plan a subject-wise evaluation on a dataset with explicit subject IDs as future work (see Section 5.3).
