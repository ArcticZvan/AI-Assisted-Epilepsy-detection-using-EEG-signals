...
\begin{itemize}
    \item \textbf{Subject-wise (patient-independent) evaluation on a dataset with explicit subject IDs.} A limitation of the current evaluation, as discussed in Section~4.6, is that the Bonn release does not include a per-file subject-ID mapping, so all cross-validation in this work is at the recording level rather than the subject level. After consulting the supervisor, the agreed plan is to address this as future work using CHB-MIT \cite{shoeb2010}, which is organised by patient (e.g., \texttt{chb01}--\texttt{chb24}) and natively supports a leave-subjects-out protocol (for example, 6 patients for training and 4 for testing). This would test whether the reported accuracy holds when the model is evaluated on previously unseen patients --- a setting closer to real clinical deployment.
